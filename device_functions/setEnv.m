...
switch HW
    ...
    case 'ninjaNIRS2021a'
        ...
        comms.LightsOff=@(app)[254 0 0 0 0 255-255 255*app.active 0 0 2*app.active+1]; %command to turn all lights off
        comms.LightsOn=@(app)[254 0 0 0 0 255-255 255*app.active 15 15 2*app.active+1]; %command to turn all lights on
        ...
        comms.SourceNOn=@(app,N,L)[254 0 0 0 0 255-255 255*app.active fliplr(bin2dec(num2str(flipud(app.LEDstate)'))') 2*app.active+1];  %Turns on source N at level L: L=0 off, L=1 high, L=-1 low
        comms.SourceNOff=@(app,N)[254 0 0 0 0 255-255 255*app.active fliplr(bin2dec(num2str(flipud(app.LEDstate)'))') 2*app.active+1];  %Turns off source N
        comms.SourceNIndL=@(app,N,L)[254 0 0 0 0 255-255 255*app.active fliplr(bin2dec(num2str(flipud(app.LEDstate)'))') 2*app.active+1];  %Only one level allowed, so L is not used in this
