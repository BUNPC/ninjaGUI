function [data,datab]=fNIRS1000_ReadBytesAvailable(s,dev,SD)
% Reads the serial port for a fNIRS1000 device. It reads N_Optodes
...
Aaux = zeros(N_AUX,1);  % buffer for aux data

data=nan(size(ML,1)+N_AUX,1); %buffer for output data
...
for k1=1:N_OPTODES
    ...
    %%% how do I implement this error handler?
    %
    %     if mod(ipacket,600) == 0
    %         % Request status packet
    %         fwrite(app.sp,[254 0 0 0 0 255-255 255-255 r_led ir_led 1]);
    %     end
    
    if raw(1)==254
        % check how this condition behaves in the real program or if it
        % even triggers at all due to the ipacket not being implemented
        ...
        continue;
    elseif (raw(1)<N_OPTODES)
        % case where data comes from optode channel
    ...
    elseif (raw(1)==200)
        % case where data comes from aux channel
        
        if raw(2) ~= N_BYTES_TO_READ_PER_SAMPLE-2
            disp(['Aux byte number annunciation error. Expecting ' num2str(N_BYTES_TO_READ_PER_SAMPLE-2) ', received ' num2str(raw(2))]);
            return
        end
        %         if raw(3) ~= mod(auxcnt,256)
        %             disp(['Aux sample counter error. Expecting ' num2str(mod(auxcnt,256)) ', received ' num2str(raw(3))]);
        %             %break;
        %         end
        %
        for iaux = 1:N_AUX
            offset = (iaux-1)*N_BYTES_PER_AUX +3;
            Aaux(iaux,1) = sum(raw((1:N_BYTES_PER_AUX) + offset) .* 256.^(0:N_BYTES_PER_AUX-1)');
        end
        %
        %         auxcnt = auxcnt +1;
...
fss=abs(fMap(ind));
dets=ML(:,2);
ind2 = sub2ind(size(datab),dets,ones(size(dets)),fss); %identifies the indices of the data packet that correspond to the measurement list

data=abs(datab(ind2));


%%concatenate intensity data with aux data
