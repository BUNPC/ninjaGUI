...
if ~isfield(stat,'s')
    stat.s = serialport("COM6", 12000000, 'FlowControl', 'hardware');
end
stat.s.flush();
stat.s.Timeout = 5;

%% Status Register
stat.v_src_boost = false;
...
stat.rama((stat.nstates+0):end,9) = 1; % mark sequence end
...
stat.ramb(1+(1:ceil(t_end_cyc/t_state_b)),10) = 1; 
...
% next LED
se = stat.n_state_b;
si = se - floor(t_src_step/t_state_b);
stat.ramb(si:se, 12) = 1;
...
stat.ramb(si-2,9) = 1; % transmit program counter A
if stat.aux_active
    stat.ramb(si-1,8) = 1; % transmit aux data
end
for ii = find([stat.detb_active, 1]) % ,1]: always read from acc
    % set source selector bits (UART Rx Mux)
    stat.ramb(si:(si+ni_bsel-1), 1:6) = repmat(bitget(ii-1, 1:6 ,'uint8'),ni_bsel,1);
    % set flow enable bit (DetB Rx En)
    stat.ramb((si+2):(si+ni_bsel-1), 7) = 1;
