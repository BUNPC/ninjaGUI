function srcram = createLEDPowerCalibrationSrcRAM_allPowerLevels( SD)
    ...
    for iS = 1:length(lstS)
        ...
        for iPower = 1:length(maxPower)
            srcram(iSrcMod, iState, 1:16) = bitget(maxPower(iPower), range(0, 16));
            srcram(iSrcMod, iState, 17:20) = bitget(iSrc*2, range(0, 4));
        ...
        for iPower = 1:length(maxPower)
            srcram(iSrcMod, iState, 1:16) = bitget(maxPower(iPower), range(0, 16));
            srcram(iSrcMod, iState, 17:20) = bitget(iSrc*2+1, range(0, 4));
            srcram(iSrcMod, iState, 21) = 0;
            srcram(iSrcMod, iState, 31) = 0;
            iState = iState + 1;
        end
